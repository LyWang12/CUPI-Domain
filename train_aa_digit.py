...
from utils.util import get_mnist_data, get_usps_data, get_svhn_data, get_mnist_m_data, add_watermark, get_augment_data
...
SOURCE = 'usps'   # mnist, usps, svhn, mnist_m
...
def Data_loader(source, target=None):
    if source == 'mnist':
        dataset_patch = add_watermark(get_mnist_data(), 32, 80)
        dataset = get_mnist_data()
        dataset_aug = get_augment_data('mnist')
        dataset_aug_patch = add_watermark(get_augment_data('mnist'))
    elif source == 'usps':
        dataset_patch = add_watermark(get_usps_data(), 32, 80)
        dataset = get_usps_data()
        dataset_aug = get_augment_data('usps')
        dataset_aug_patch = add_watermark(get_augment_data('usps'))
    elif source == 'svhn':
        dataset_patch = add_watermark(get_svhn_data(), 32, 80)
        dataset = get_svhn_data()
        dataset_aug = get_augment_data('svhn')
        dataset_aug_patch = add_watermark(get_augment_data('svhn'))
    elif source == 'mnist_m':
        dataset_patch = add_watermark(get_mnist_m_data(), 28, 80)
        dataset = get_mnist_m_data()
        dataset_aug = get_augment_data('mnist_m')
        dataset_aug_patch = add_watermark(get_augment_data('mnist_m'))

    return dataset, dataset_patch, dataset_aug, dataset_aug_patch



def train():
    # Load datasets
    dataset, dataset_patch, dataset_aug, dataset_aug_patch = Data_loader(source=SOURCE)

    # Load val datasets
    data1_patch = add_watermark(get_mnist_data(), 32, 80)
    data2_patch = add_watermark(get_usps_data(), 32, 80)
    data3_patch = add_watermark(get_svhn_data(), 32, 80)
    data4_patch = add_watermark(get_mnist_m_data(), 28, 80)
    ...
    datafile = Cus_Dataset(mode='author',
                           ...
                           dataset_4=dataset4, begin_ind4=0, size4=TRAIN_SIZE)

    dataloader = DataLoader(datafile, batch_size=batch_size, shuffle=True, num_workers=workers, drop_last=True)

    valloader = []
    datasets = [data1_patch, data2_patch, data3_patch, data4_patch, dataset1, dataset2, dataset3, dataset4]
    for i in datasets:
        datafile_val = Cus_Dataset(mode='val', dataset_1=i, begin_ind1=TRAIN_SIZE, size1=VAL_SIZE)
