...
from utils.util import get_mnist_data, get_usps_data, get_svhn_data, get_mnist_m_data
...
TARGET = 'svhn'   # mnist, usps, svhn, mnist_m
...
def Data_loader(source, target=None):
    if source == 'mnist':
        ...
        dataset1 = get_usps_data()
    elif source == 'svhn':
        dataset1 = get_svhn_data()
    elif source == 'mnist_m':
        dataset1 = get_mnist_m_data()

    # Load target datasets
    if target == 'mnist':
        dataset2 = get_mnist_data()
        dataset3 = get_mnist_data()
    elif target == 'usps':
        dataset2 = get_usps_data()
        dataset3 = get_usps_data()
    elif target == 'svhn':
        dataset2 = get_svhn_data()
        dataset3 = get_svhn_data()
    elif target == 'mnist_m':
        dataset2 = get_mnist_m_data()
        dataset3 = get_mnist_m_data()
...
def train():
    ...
    PATH = '/data1/WLY/code/PAMI2/git/model_save/ts/digit/' + SOURCE + '_to_' + TARGET + '.pth'
