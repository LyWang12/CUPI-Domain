...
class CUPI(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            conv_mean = nn.Conv1d(1, 1, 1, bias=False).to(device)
            conv_std = nn.Conv1d(1, 1, 1, bias=False).to(device)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, cupi=False):
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=True, pro_dim=768,
        ...
        self.head = nn.Linear(self.num_features, self.pro_dim) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x, y=None, z=None, action=None):
        if action == 'val':
            ...
            x, H, W = self.patch_embed(x)
            x = self.pos_drop(x)
            x1, H, W = self.layers[0](x, H, W, cupi=False)
            x2, H, W = self.layers[1](x1, H, W, cupi=False)
            x3, H, W = self.layers[2](x2, H, W, cupi=False)
            x4, H, W = self.layers[3](x3, H, W, cupi=False)
            ...
            x, H, W = self.patch_embed(x)
            x = self.pos_drop(x)
            x1, H, W = self.layers[0](x, H, W, cupi=True)
            x2, H, W = self.layers[1](x1, H, W, cupi=True)
            x3, H, W = self.layers[2](x2, H, W, cupi=True)
            x4, H, W = self.layers[3](x3, H, W, cupi=True)
            x = self.norm(x4)  # [B, L, C]
            x = self.avgpool(x.transpose(1, 2))  # [B, C, 1]
            x = torch.flatten(x, 1)
            return x1, x2, x3, x4, x
