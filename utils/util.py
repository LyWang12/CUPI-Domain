...
def get_home_data(source):
    ...
    list_label = []
    data_size = 0
    root_temp = r"./data/office-home-65/{}".format(source)
    class_path = os.listdir(root_temp)
    for i in range(len(class_path)):
        class_temp = os.path.join(root_temp, class_path[i])
        img_path = os.listdir(class_temp)
...
def get_imagenet_data(source):
    list_img = []
    list_label = []
    data_size = 0
    root_temp = r"./data/DomainNet/{}".format(source)
    class_path = os.listdir(root_temp)
    for i in range(len(class_path)):
        class_temp = os.path.join(root_temp, class_path[i])
        img_path = os.listdir(class_temp)
...
def get_augment_data(source):
    ...
    augment_trainset = os.listdir("./data/augment_{}".format(source))
    augment_labels = np.loadtxt("./data//augment_{}/labels".format(source))
    for i in range(len(augment_trainset) - 1):
        img = cv2.imread("./data//augment_{}/img_{}.png".format(source, i))
        list_img.append(img)
        list_label.append(np.eye(10)[int(augment_labels[i])])
        aug_data_size += 1

    root_temp = r"./data/AdaIN/output/{}".format(source)
    class_path = os.listdir(root_temp)
    for i in range(len(class_path)):
        class_temp = os.path.join(root_temp, class_path[i])
        img_path = os.listdir(class_temp)
